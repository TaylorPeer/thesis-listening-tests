...
def show_intro_text():
    ...
    st.write(
        "This is an open survey evaluating the quality of drum patterns generated by different artificial intelligence methods.")
    st.write(
...
st.write(
    "A drum pattern (or beat) is a short rhythmical sequence that typically sets the time signature and groove of a song. For example, the following recording demonstrates a simple *breakbeat* pattern:")

example_file = open("089 Urban 01.wav", 'rb')
example_audio_bytes = example_file.read()
st.audio(example_audio_bytes, format='audio/wav')

st.write(
    "Below you can listen to and rate various drum patterns. Each pattern was either written by a human composer or generated by an AI algorithm. All patterns have been converted to music notation and reproduced via a synthetic drum kit to obscure their original source.")
            ...
            with st.spinner("Submitting rating, hold on for a second..."):
                ...
                if st.session_state.submit_button_text == "Submit":
                    ...
                    if not nothing_selected:
                        st.session_state.submissions = st.session_state.submissions + 1
                        st.session_state.progress = st.session_state.submissions / 20
                        if st.session_state.submissions >= 20:
                            st.session_state.progress = 100
                    if 20 > st.session_state.submissions > 0:
                        st.session_state.progress_text = "{} of 20 suggested ratings completed".format(st.session_state.submissions)
                    elif st.session_state.submissions >= 20:
                        st.session_state.progress_text = "You've already completed the 20 suggested ratings but feel free to rate more. Every rating helps improve the dataset."
                else:
                    st.session_state.feedback = ""
                    st.session_state.score = ""
                    st.session_state.submit_button_text = "Submit"
